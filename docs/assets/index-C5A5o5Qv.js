...
{c}%`},labelLine:{lineStyle:{color:"#4b5563"}},data:t.length>0?t:[{name:"未选择",value:100,itemStyle:{color:"#1f2937"}}],itemStyle:{borderColor:"#0a0e17",borderWidth:2,color:o=>e[o.dataIndex%e.length]}}]};N.setOption(n,!0)}function oe(){A==null||A.dispose(),A=null,H==null||H.dispose(),H=null,N==null||N.dispose(),N=null}function kt(t,e){if(t.length<e)return[];const n=new Array(t.length).fill(null);let o=0;for(let a=0;a<t.length;a++)o+=t[a],a>=e&&(o-=t[a-e]),a>=e-1&&(n[a]=parseFloat((o/e).toFixed(2)));return n}function se(t,e=14){if(t.length<e+1)return[];const n=new Array(t.length).fill(null),o=[],a=[];for(let i=1;i<t.length;i++){const l=t[i]-t[i-1];o.push(l>0?l:0),a.push(l<0?-l:0)}let s=o.slice(0,e).reduce((i,l)=>i+l,0)/e,r=a.slice(0,e).reduce((i,l)=>i+l,0)/e;for(let i=e;i<o.length;i++){if(r===0)n[i+1]=100;else{const l=s/r;n[i+1]=parseFloat((100-100/(1+l)).toFixed(1))}s=(s*(e-1)+o[i])/e,r=(r*(e-1)+a[i])/e}return n}let tt=[],R=[],Z="a-share",nt="all",$t=!1,Mt=!1,Lt=!1,Et=!1,Rt=!1;function K(){M.holdings=R.map(t=>({code:t.code,name:t.name,sector:t.sector,market:t.market,weight:t.weight}))}function Ht(){var n;R=[],K(),Z="a-share",nt="all",M.stocksData&&ae(M.stocksData),$t||(document.querySelectorAll(".market-tab").forEach(o=>{o.addEventListener("click",()=>{document.querySelectorAll(".market-tab").forEach(a=>a.classList.remove("active")),o.classList.add("active"),Z=o.dataset.market,document.getElementById("stock-search").value="",G()})}),$t=!0),Rt||((n=document.getElementById("btn-random"))==null||n.addEventListener("click",le),Rt=!0);const t=document.getElementById("stock-search");if(t&&!Lt){let o=null;t.addEventListener("input",()=>{clearTimeout(o),o=setTimeout(()=>{const a=t.value.trim().toLowerCase();a&&(Z="all",document.querySelectorAll(".market-tab").forEach(s=>s.classList.remove("active"))),G(a)},250)}),Lt=!0}Mt||(document.querySelectorAll(".period-btn").forEach(o=>{o.addEventListener("click",()=>{document.querySelectorAll(".period-btn").forEach(s=>s.classList.remove("active")),o.classList.add("active");const a=o.dataset.period;a==="custom"?(document.getElementById("custom-period-wrap").classList.remove("hidden"),M.period="custom"):(document.getElementById("custom-period-wrap").classList.add("hidden"),M.period=a)})}),Mt=!0);const e=document.getElementById("custom-months");if(e&&!Et){const o=()=>{let a=parseInt(e.value);(isNaN(a)||a<1)&&(a=1),a>120&&(a=120),e.value=a,M.customMonths=a};e.addEventListener("input",o),e.addEventListener("blur",o),Et=!0}zt(),W()}function ae(t){tt=t.stocks,re(t.sectors),G(),zt()}function re(t){const e=document.getElementById("sector-filters");if(!e)return;e.innerHTML="";const n=document.createElement("button");n.className="sector-btn active",n.textContent="全部",n.addEventListener("click",()=>{nt="all",document.querySelectorAll(".sector-btn").forEach(o=>o.classList.remove("active")),n.classList.add("active"),G()}),e.appendChild(n),t.forEach(o=>{const a=document.createElement("button");a.className="sector-btn",a.textContent=o,a.addEventListener("click",()=>{nt=o,document.querySelectorAll(".sector-btn").forEach(s=>s.classList.remove("active")),a.classList.add("active"),G()}),e.appendChild(a)})}function G(t){const e=document.getElementById("stock-grid");if(!e)return;let n=tt;if(t){const o=t.toLowerCase();n=tt.filter(a=>a.name.toLowerCase().includes(o)||a.code.toLowerCase().includes(o)).slice(0,50)}else Z==="all"&&(Z="a-share"),n=tt.filter(o=>{const a=o.market===Z,s=nt==="all"||o.sector===nt;return a&&s});n.sort((o,a)=>a.marketCap-o.marketCap),e.innerHTML=n.map(o=>{var i;const a=R.find(l=>l.code===o.code),s=o.latestPrice;return`
  ...
  `,document.body.appendChild(m),(B=m.querySelector(".stock-modal-close"))==null||B.addEventListener("click",()=>m.remove()),(k=m.querySelector(".stock-modal-add"))==null||k.addEventListener("click",function(){const{code:x,name:h,sector:w,market:L}=this.dataset;Ot({code:x,name:h,sector:w,market:L}),m.remove()}),m.addEventListener("click",x=>{x.target===m&&m.remove()}),setTimeout(()=>{const x=document.getElementById(`tech-indicators-${e.code}`);if(!x)return;const h=e.prices.slice(-120),w=kt(h,20),L=kt(h,60),$=se(h,14);h[h.length-1];const v=w[w.length-1],y=L[L.length-1],b=$[$.length-1],D=v>y?"📈 多头排列":"📉 空头排列",T=b>70?"⚠️ 超买":b<30?"💡 超卖":"➖ 中性";x.innerHTML=`
  ...
  `).join("");const n=R.reduce((s,r)=>s+r.weight,0);e.textContent=`合计: ${n}%`,e.className=n===100?"text-sm font-mono text-neon-green":"text-sm font-mono text-neon-red";const o=((a=document.getElementById("lock-weights"))==null?void 0:a.checked)||!1;t.querySelectorAll('[data-action="weight"]').forEach(s=>{s.addEventListener("input",r=>{const i=parseInt(s.dataset.index);R[i].weight=parseInt(r.target.value);const l=t.querySelector(`[data-action="weight-input"][data-index="${i}"]`);l&&(l.value=r.target.value),o?(K(),W(),Y(),V()):It(i,parseInt(r.target.value))}),s.addEventListener("change",r=>{if(!o)return;const i=parseInt(s.dataset.index);R[i].weight=parseInt(r.target.value),K(),W(),Y(),V()})}),t.querySelectorAll('[data-action="weight-input"]').forEach(s=>{s.addEventListener("change",r=>{const i=parseInt(s.dataset.index);let l=parseInt(r.target.value)||1;l=Math.max(1,Math.min(95,l)),R[i].weight=l;const c=t.querySelector(`[data-action="weight"][data-index="${i}"]`);c&&(c.value=l),o?(K(),W(),Y(),V()):It(i,l)})}),t.querySelectorAll('[data-action="remove"]').forEach(s=>{s.addEventListener("click",()=>{const r=parseInt(s.dataset.index);R.splice(r,1),xt(),K(),G(),W(),Y(),V()})})}function It(t,e){const n=R.filter((r,i)=>i!==t);if(n.length===0)return;R[t].weight=e;const o=100-e,a=n.reduce((r,i)=>r+i.weight,0);if(a===0){const r=Math.floor(o/n.length);n.forEach(l=>l.weight=r);const i=n.reduce((l,c)=>l+c.weight,0);n[0].weight+=o-i}else{const r=o/a;let i=0;n.forEach((d,u)=>{d.weight=Math.max(1,Math.round(d.weight*r)),i+=d.weight});let l=o-i,c=0;for(;l!==0&&c<20;){c++;for(const d of n)if(l>0?(d.weight++,l--):l<0&&d.weight>1&&(d.weight--,l++),l===0)break}l!==0&&n.length>0&&(n[0].weight=Math.max(1,n[0].weight+l))}const s=R.reduce((r,i)=>r+i.weight,0);s!==100&&R.length>0&&(R[0].weight+=100-s),K(),W(),Y(),V()}function Y(){const t={};R.forEach(n=>{t[n.sector]=(t[n.sector]||0)+n.weight});const e=Object.entries(t).map(([n,o])=>({name:n,value:o}));ne(e)}function le(){var n;R=[];const t=4+Math.floor(Math.random()*4),e=[...tt].sort(()=>Math.random()-.5);for(let o=0;o<Math.min(t,e.length);o++){const a=e[o];R.push({code:a.code,name:a.name,sector:a.sector,market:a.market,weight:0})}xt(),document.getElementById("stock-search").value="",Z="a-share",document.querySelectorAll(".market-tab").forEach(o=>o.classList.remove("active")),(n=document.querySelector('[data-market="a-share"]'))==null||n.classList.add("active"),G(),W(),Y(),V(),showToast(`🎲 随机选中 ${R.length} 只股票，看看运气如何？`)}function qt(){return R.map(t=>({code:t.code,weight:t.weight}))}function ce(){return M.period==="custom"?"custom"+(M.customMonths||18):M.period}let St=!1;function de(t){var i,l;const{results:e,amount:n,leverage:o}=t,a=n||1e5,s=o||1,r=[...e].sort((c,d)=>c.rank-d.rank);ue(r,a,s),te("chart-returns",r,a,s),me(r,a,s),St||((i=document.getElementById("chart-mode-pct"))==null||i.addEventListener("click",()=>wt("pct")),(l=document.getElementById("chart-mode-value"))==null||l.addEventListener("click",()=>wt("value")),St=!0)}function ue(t,e,n){const o=document.getElementById("ranking-table");if(!o)return;const a=["🥇","🥈","🥉"];o.innerHTML=t.map((s,r)=>{const i=s.isUser,l=r<3?a[r]:s.rank,c=s.totalReturn>=0?"text-neon-red":"text-neon-green",d=i?"user-highlight":"",u=s.totalReturn*n,m=parseFloat(Math.max(-100,u).toFixed(1)),p=Math.round(e*m/100),g=(m>=0?"+":"")+Number(p).toLocaleString(),I=parseFloat((s.maxDrawdown*n).toFixed(1));let S="";if(!i&&s.holdingsDetail&&s.holdingsDetail.length>0){const B=s.holdingsDetail.map(k=>`<div class="flex justify-between text-xs py-1">
    ...
    `}).join(""),window.toggleHoldings||(window.toggleHoldings=function(s){const r=document.getElementById(`holdings-${s}`);r&&r.classList.toggle("hidden")})}function me(t,e,n){const o=document.getElementById("metrics-table");if(!o)return;const a=["基金","累计收益","年化收益","最大回撤","夏普比率","胜率"],s=t.map(r=>{const i=r.totalReturn>=0?"metric-up":"metric-down",l=r.totalReturn*n,c=parseFloat(Math.max(-100,l).toFixed(1)),d="★".repeat(r.fundRating||0)+"☆".repeat(5-(r.fundRating||0)),u=r.isUser?`
      ...
      <table class="metrics-table w-full text-xs">
        <thead>
          <tr class="border-b border-dark-500/30">
            ${a.map(r=>`<th class="px-3 py-2 text-left font-medium whitespace-nowrap cursor-help" title="${ge(r)}">${r}</th>`).join("")}
          </tr>
        </thead>
        <tbody>
          ${s}
...
• ★☆☆☆☆：<1分，需谨慎`}}[t];if(!n)return;(l=document.querySelector(".metric-modal-overlay"))==null||l.remove();const o=document.createElement("div");o.className="metric-modal-overlay fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm",o.innerHTML=`
    ...
    `,document.body.appendChild(o);const a=o.querySelector(".metric-modal-close"),s=o.querySelector("button"),r=()=>o.remove();a==null||a.addEventListener("click",r),s==null||s.addEventListener("click",r),o.addEventListener("click",c=>{c.target===o&&r()});const i=c=>{c.key==="Escape"&&(r(),document.removeEventListener("keydown",i))};document.addEventListener("keydown",i)});function he(t,e=null){const{styleTag:n,matchPerson:o,matchPersonDesc:a,matchPersonOrg:s,metrics:r,radarData:i,commentary:l}=t,c=document.getElementById("diagnosis-tag");c&&(c.innerHTML=`
    ...
    `),ee("chart-radar",i,"你的基金");const u=document.getElementById("commentary-text");u&&(e!=null&&e.results&&e.results.length>0?Nt(u,e.results,e.errors):e!=null&&e.loading?pe(u):e!=null&&e.errors&&e.errors.length>0?xe(u,l,e.errors):ve(u,l)),be(r)}function fe(t){const e=document.getElementById("commentary-text");e&&(t.results&&t.results.length>0?Nt(e,t.results,t.errors):t.errors&&t.errors.length>0&&e.innerHTML.includes("loading-dots")&&ye(e,t.errors))}function Nt(t,e,n){let o=e.map((a,s)=>`<div class="llm-result mb-3">
      <div class="text-white leading-relaxed text-sm md:text-base">${a.text.replace(/\*\*(.+?)\*\*/g,'<strong class="text-neon-blue">$1</strong>').replace(/\*(.+?)\*/g,"<em>$1</em>").split(`
...
`).map(i=>i.trim()).filter(Boolean).map(i=>`<p style="margin-bottom:10px;line-height:1.8;">${i.replace(/\n/g,"<br>")}</p>`).join("")}</div>
  ...
  `}function xe(t,e,n){let o="";if(e){const a=e.split(`
...
`).map(o=>o.trim()).filter(Boolean);t.innerHTML=n.map((o,a)=>'<p style="margin-bottom:'+(a<n.length-1?"12px":"0")+';line-height:1.8;">'+o+"</p>").join(""),t.innerHTML+='<div class="mt-3 text-right text-xs text-gray-500">📋 离线模板点评 · 仅供参考</div>'}function ye(t,e){t.innerHTML=`
    <div class="py-2">
  ...
  `}function be(t){const e=document.getElementById("ai-commentary");if(!e)return;let n=document.getElementById("metrics-summary");n&&n.remove(),n=document.createElement("div"),n.id="metrics-summary";const o=(r,i)=>{if(r==null||isNaN(r))return"-";const l=Math.pow(10,i);return Math.round(r*l)/l},a="★".repeat(t.fundRating||0)+"☆".repeat(5-(t.fundRating||0)),s=t.fundRating>=4?"text-gold-400":t.fundRating>=3?"text-neon-blue":"text-gray-400";n.innerHTML=`
...
💡 **专属建议**：`);const v=[];return I?(v.push("🚨 立即退出所有杠杆仓位，本金没了就什么都没了。"),v.push("📚 建议先学习《聪明的投资者》等经典书籍。"),v.push("🎮 先用模拟盘练习至少3个月。")):B||k?(v.push("🛑 暂停加仓，不要继续摊低成本。"),v.push("🔍 仔细分析每只股票的买入逻辑。"),S&&v.push("📉 降低杠杆至1x或2x。")):x?v.push("🤔 微调策略，优化选股标准。"):h?v.push("📊 加入债券ETF等低风险资产平滑曲线。"):(w||L)&&v.push("💰 适当减仓，锁定部分利润。"),s[1]>60&&v.push(`🔄 ${s[0]}占比过高，建议减仓分散。`),p===1&&!I&&v.push("🌍 建议配置其他市场分散风险。"),e.maxDrawdown>30&&!I&&v.push("🛡️ 设置止损线（如-15%）并严格执行。"),i.push(...v.map((b,D)=>`${D+1}. ${b}`)),i.push(`
...
${d}`}function De(t){if(!t)return"";const e=[/最终回答[：:]\s*/,/最终点评[：:]\s*/,/以下是点评[：:]\s*/,/点评如下[：:]\s*/,/回复[：:]\s*\n/];for(const a of e){const s=t.match(a);if(s){const r=t.slice(s.index+s[0].length).trim();if(r.length>50)return r}}const n=t.split(/\n\n+/).filter(a=>a.trim());if(n.length>=3)for(let a=n.length-1;a>=0;a--){const s=n[a].trim();if(!/^(我们|首先|需要|用户|任务|好的|让我|我来|根据|以上|下面|现在)/.test(s)&&s.length>20)return n.slice(a).join(`

`)}const o=t.replace(/^.*?我们需要.*?\n/s,"").trim();return o.length>50?o:t.trim()}async function Pe(t){var a,s;if(!it.apiKey)return Q.push({api:"API",error:"未配置 apiKey（请创建 config.js 并填入 Key）"}),null;const e=it.baseUrl||"/api",n=new AbortController,o=setTimeout(()=>n.abort(),Pt);try{const r=await fetch(`${e}/chat/completions`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${it.apiKey}`},body:JSON.stringify({model:it.model,messages:[{role:"system",content:"你是东方财富的资深基金经理。直接输出点评正文，禁止输出思考过程。少用引号，适度emoji，结尾自然引导到东方财富产品。"},{role:"user",content:t}],max_tokens:2e3,temperature:.8}),signal:n.signal});if(!r.ok){const d=await r.text().catch(()=>"");return Q.push({api:"API",error:`HTTP ${r.status}${d?": "+d.slice(0,200):""}`}),null}const l=(s=(a=(await r.json()).choices)==null?void 0:a[0])==null?void 0:s.message;let c=((l==null?void 0:l.content)||"").trim();return!c&&(l!=null&&l.reasoning_content)&&(c=De(l.reasoning_content)),c&&!c.includes("东方财富")&&(c+=Gt?`

赚了别藏着，去东方财富股吧晒晒操作，让韭菜们学学！`:`

亏了别慌，下载东方财富APP，权威·专业·及时，帮你找回场子！`),c||Q.push({api:"API",error:"返回内容为空（模型未输出有效回复）"}),c||null}catch(r){const i=r.name==="AbortError"?`请求超时（${Pt/1e3}秒）。请检查：1) 是否在公司内网 2) Vite 代理是否正常 3) API 地址是否正确`:r.message||"网络错误";return Q.push({api:"API",error:i}),console.warn("[LLM]",r.message),null}finally{clearTimeout(o)}}const M={currentScreen:"builder",fundName:"",holdings:[],period:"1y",customMonths:18,backtestResults:null,stocksData:null,userResult:null,investAmount:1e5,leverage:1};let lt=null,ft=null;function Be(){const t=document.getElementById("particle-canvas");if(!t)return;const e=t.getContext("2d");let n=[];lt&&(cancelAnimationFrame(lt),lt=null),ft&&window.removeEventListener("resize",ft);function o(){t.width=window.innerWidth,t.height=window.innerHeight}o(),ft=o,window.addEventListener("resize",o);class a{constructor(){this.reset()}reset(){this.x=Math.random()*t.width,this.y=Math.random()*t.height,this.size=Math.random()*2+.5,this.speedX=(Math.random()-.5)*.3,this.speedY=(Math.random()-.5)*.3,this.opacity=Math.random()*.5+.1}update(){this.x+=this.speedX,this.y+=this.speedY,(this.x<0||this.x>t.width||this.y<0||this.y>t.height)&&this.reset()}draw(){e.beginPath(),e.arc(this.x,this.y,this.size,0,Math.PI*2),e.fillStyle=`rgba(79, 195, 247, ${this.opacity})`,e.fill()}}for(let r=0;r<80;r++)n.push(new a);function s(){e.clearRect(0,0,t.width,t.height),n.forEach(r=>{r.update(),r.draw()});for(let r=0;r<n.length;r++)for(let i=r+1;i<n.length;i++){const l=n[r].x-n[i].x,c=n[r].y-n[i].y,d=Math.sqrt(l*l+c*c);d<120&&(e.beginPath(),e.moveTo(n[r].x,n[r].y),e.lineTo(n[i].x,n[i].y),e.strokeStyle=`rgba(79, 195, 247, ${.08*(1-d/120)})`,e.lineWidth=.5,e.stroke())}lt=requestAnimationFrame(s)}s()}function vt(t){oe(),document.querySelectorAll(".screen").forEach(o=>o.classList.remove("active","hidden")),document.querySelectorAll(".screen").forEach(o=>o.classList.add("hidden"));const e=document.getElementById(`screen-${t}`);e&&(e.classList.remove("hidden"),e.classList.add("active"),e.scrollIntoView({behavior:"smooth",block:"start"})),M.currentScreen=t;const n=document.getElementById("header");n&&(n.style.display=t==="builder"?"":"none")}async function Ut(){try{const e=await fetch("./"+"stocks.json");if(!e.ok)throw new Error(`HTTP ${e.status}`);const n=await e.json();return n.stocks.forEach(o=>{o.latestPrice=o.prices[o.prices.length-1]}),M.stocksData=n,M.stocksData}catch(t){return console.error("Failed to load stocks:",t),null}}async function Ce(){var e,n,o,a;const t=document.getElementById("btn-start");t.disabled=!0,t.textContent="⏳ 回测计算中...";try{const s=qt(),r=ce();let i=parseFloat((e=document.getElementById("invest-amount"))==null?void 0:e.value)||1e5;i=Math.max(100,Math.min(1e8,i));const l=parseFloat((n=document.getElementById("leverage"))==null?void 0:n.value)||1;let c=M.fundName||((a=(o=document.getElementById("fund-name"))==null?void 0:o.value)==null?void 0:a.trim());c||(c=Ae(s,M.stocksData)),M.stocksData||await Ut();const d=Wt(M.stocksData,s,r);d.label=c,d.amount=i,d.leverage=l,M.userResult=d,M.holdings=s,M.investAmount=i,M.leverage=l;const u=$e(M.stocksData,r),m=[d,...u];m.sort((I,S)=>S.totalReturn-I.totalReturn),m.forEach((I,S)=>{I.rank=S+1}),M.backtestResults=m,vt("arena"),de({fundName:c,period:r,results:m,amount:i,leverage:l});const p={...d,totalReturn:d.totalReturn*l,maxDrawdown:d.maxDrawdown*l},g=Vt(M.stocksData,s,p);g.metrics.leverage=l,Re(g)}catch(s){ze("回测失败："+s.message,"error"),t.disabled=!1,t.textContent="⚡ 开始挑战"}}async function Fe(){vt("diagnosis");const t={...M.userResult,totalReturn:M.userResult.totalReturn*M.leverage,maxDrawdown:M.userResult.maxDrawdown*M.leverage},e=Vt(M.stocksData,M.holdings,t);e.metrics.leverage=M.leverage;const n=Ie();let o={results:null,errors:[],loading:n};n||(o=await Bt()),he(e,o),n&&(o=await Bt(),fe(o))}function Te(){M.fundName="",M.holdings=[],M.backtestResults=null,M.userResult=null,document.getElementById("fund-name").value="",document.getElementById("btn-start").disabled=!0,vt("builder"),Ht()}function Ae(t,e){var $;if(!t||t.length===0)return"我的基金";const n={};e&&e.stocks&&e.stocks.forEach(v=>{n[v.code]=v});const o={},a={};let s=!1,r=!1,i=!1,l=!1;t.forEach(v=>{const y=n[v.code];y&&(o[y.market]=(o[y.market]||0)+v.weight,a[y.sector]=(a[y.sector]||0)+v.weight,y.sector==="科技"&&(s=!0),y.sector==="金融"&&(r=!0),y.sector==="消费"&&(i=!0),y.sector==="医药"&&(l=!0))});const c=Object.entries(o).sort((v,y)=>y[1]-v[1]),d=(($=c[0])==null?void 0:$[0])||"a-share",u=c.length,m={"a-share":["华夏","国泰","南方","易方达","嘉实","博时","广发","富国"],hk:["港股","香港","恒生","中港","沪港深"],us:["纳斯达克","标普","美股","全球","海外"],index:["指数","ETF","被动"]};let p;u>=3?p=["全球","国际","环球","世界","跨市场"]:u===2?p=["沪港深","深港通","AH","中美","跨市场"]:p=m[d]||m["a-share"];let g=[];s&&t.length<=3?g=["创新","科技","成长","新兴","前沿","智能"]:r&&t.length<=3?g=["金融","价值","蓝筹","红利","稳健","精选"]:i&&t.length<=3?g=["消费","品质","生活","品牌","升级"]:l&&t.length<=3?g=["健康","医疗","生命","医药","生物"]:t.length>=8?g=["优选","精选","配置","均衡","多元","全能"]:t.length<=3?g=["聚焦","集中","核心","龙头","精选","优势"]:g=["成长","价值","均衡","轮动","趋势","精选","优选","灵活"];const I=["混合","股票","配置","优选","精选","成长","价值","稳健","进取","灵活"],S=p[Math.floor(Math.random()*p.length)],B=g[Math.floor(Math.random()*g.length)],k=I[Math.floor(Math.random()*I.length)],x=[S+B+k,S+k+B,B+k,S+B],h=x[Math.floor(Math.random()*x.length)],w=["超级","至尊","王者","巅峰","传奇","无敌","神级","霸道"];return w[Math.floor(Math.random()*w.length)]+h+"（您）"}async function je(){Be(),await Ut(),Ht();const t=document.getElementById("invest-amount");t&&(t.addEventListener("input",n=>{let o=parseInt(n.target.value)||1e5;o=Math.max(100,Math.min(1e8,o)),n.target.value&&parseInt(n.target.value)!==o&&(n.target.value=o)}),t.addEventListener("blur",n=>{let o=parseInt(n.target.value)||1e5;o=Math.max(100,Math.min(1e8,o)),n.target.value=o})),document.getElementById("btn-start").addEventListener("click",Ce),document.getElementById("btn-diagnosis").addEventListener("click",Fe),document.getElementById("btn-restart").addEventListener("click",Te),document.getElementById("fund-name").addEventListener("input",n=>{M.fundName=n.target.value.trim(),V()});const e=document.getElementById("leverage");e&&e.addEventListener("input",()=>{document.getElementById("leverage-display").textContent=e.value+"x"})}je();let pt=null;function ze(t,e="info"){const n=document.getElementById("toast-msg");n&&n.remove(),pt&&clearTimeout(pt);const o=e==="error"?"bg-red-500/90":"bg-green-500/90",a=document.createElement("div");a.id="toast-msg",a.className=`fixed top-4 left-1/2 -translate-x-1/2 ${o} text-white px-6 py-3 rounded-xl shadow-lg z-50 text-sm`,a.textContent=t,document.body.appendChild(a),pt=setTimeout(()=>{a.style.opacity="0",a.style.transition="opacity .3s",setTimeout(()=>a.remove(),300)},3e3)}function V(){const t=document.getElementById("btn-start");t&&(t.disabled=qt().length<1)}
