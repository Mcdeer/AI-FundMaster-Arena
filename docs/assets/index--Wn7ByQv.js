(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))o(r);new MutationObserver(r=>{for(const s of r)if(s.type==="childList")for(const a of s.addedNodes)a.tagName==="LINK"&&a.rel==="modulepreload"&&o(a)}).observe(document,{childList:!0,subtree:!0});function n(r){const s={};return r.integrity&&(s.integrity=r.integrity),r.referrerPolicy&&(s.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?s.credentials="include":r.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function o(r){if(r.ep)return;r.ep=!0;const s=n(r);fetch(r.href,s)}})();let A=null,H=null,N=null,Ct="pct",Ft=[],Tt=1e5,At=1;function _(t,e){if(t==null||isNaN(t))return t;const n=Math.pow(10,e);return Math.round(t*n)/n}function Qt(t,e){const n=t.length;if(n<10)return[];const o=(n-1)/2,r=t.reduce((d,u)=>d+u,0)/n;let s=0,a=0;for(let d=0;d<n;d++)s+=(d-o)*(t[d]-r),a+=(d-o)*(d-o);const i=a!==0?s/a:0,l=t[n-1],c=[];for(let d=1;d<=e;d++){const u=(Math.random()-.5)*Math.abs(i)*d*.5;c.push(_(l+i*d+u,2))}return c}function Zt(t,e){const n=new Date,o=[],r=t-e;for(let s=0;s<r;s++){const a=new Date(n);a.setDate(a.getDate()-(r-s)),s===0||s===r-1||s%Math.max(1,Math.floor(r/6))===0?o.push(a.getMonth()+1+"/"+a.getDate()):o.push("")}for(let s=0;s<e;s++){const a=new Date(n);a.setDate(a.getDate()+s+1),s===0||s===e-1||s%Math.max(1,Math.floor(e/2))===0?o.push("🔮"+(a.getMonth()+1)+"/"+a.getDate()):o.push("")}return o}function te(t,e,n,o){const r=document.getElementById(t);r&&(Ft=e,Tt=n||1e5,At=o||1,A&&A.dispose(),A=echarts.init(r),jt())}function jt(){const t=Ft,e=Tt,n=At,o=Ct==="value",r="#4fc3f7",s=["#69f0ae","#f0c060","#b388ff","#ff80ab","#18ffff","#ffab40","#ff5252"],a=[];let i=0,l=0;const c=t.find(h=>h.isUser);c&&(l=c.chartData.length),t.forEach((h,w)=>{h.chartData.length>i&&(i=h.chartData.length)});let d=[],u=-1;if(c){const h=c.chartData;for(let w=0;w<h.length;w++)if((h[w]-100)*n<=-100){u=w;break}if(u<0){const w=Qt(c.chartData,Math.max(1,Math.floor(i*.05)));w.length>0&&(d=w,i=Math.max(i,l+d.length))}}let m=[];const p=t.find(h=>h.isUser&&h.dateLabels);if(p&&p.dateLabels)m=[...p.dateLabels];else{const h=Math.max(0,i-l);m=Zt(i,h)}if(d.length>0&&m.length>0){const h=m[m.length-1],[w,M]=h.split("/").map(Number);for(let $=1;$<=d.length;$++){const v=new Date(2026,w-1,M);v.setDate(v.getDate()+$);const y=$===1||$===d.length||$%Math.max(1,Math.floor(d.length/3))===0?"🔮"+(v.getMonth()+1)+"/"+v.getDate():"";m.push(y)}}t.forEach((h,w)=>{const M=h.isUser,$=h.isBenchmark,v=h.chartData,y=M?r:s[(w-1)%s.length];let b=[],D=-1;if(o)for(let E=0;E<v.length;E++){if((v[E]-100)*n<=-100){D=E,b.push(0);break}b.push(_(e*n*v[E]/100,0))}else for(let E=0;E<v.length;E++){const C=_((v[E]-100)*n,1);if(C<=-100){D=E,b.push(-100);break}b.push(C)}for(;b.length<i;)b.push(null);let T=[...b];if(M&&d.length>0&&!o)for(let E=0;E<d.length;E++){const C=_((d[E]-100)*n,1);l+E<T.length?T[l+E]=C:T.push(C)}else if(M&&d.length>0&&o)for(let E=0;E<d.length;E++){const C=_(e*n*d[E]/100,0);l+E<T.length?T[l+E]=C:T.push(C)}if(M&&d.length>0&&D<0){const E=T.slice(0,l),C=new Array(l-1).fill(null),ct=E[E.length-1];C.push(ct);for(let U=0;U<d.length;U++){const dt=o?_(e*n*d[U]/100,0):_((d[U]-100)*n,1);C.push(dt)}a.push({name:h.label,type:"line",data:E,smooth:!0,symbol:"none",lineStyle:{width:4,type:"solid",color:y},itemStyle:{color:y},emphasis:{focus:"series",lineStyle:{width:6}},z:10,endLabel:{show:!0,formatter:h.label,color:y,fontSize:11,offset:[10,0]}}),a.push({name:"预测走势",type:"line",data:C,smooth:!0,symbol:"none",lineStyle:{width:3,type:"dashed",color:y,opacity:.7},itemStyle:{color:y},z:9,silent:!0})}else a.push({name:h.label,type:"line",data:T,smooth:!0,symbol:"none",lineStyle:{width:M?4:$?1.5:2,type:"solid",color:y,opacity:$?.5:1},itemStyle:{color:y},emphasis:{focus:"series",lineStyle:{width:M?6:3}},z:M?10:1,endLabel:M?{show:!0,formatter:h.label,color:y,fontSize:11,offset:[10,0]}:void 0,...D>=0?{markPoint:{data:[{name:"💥",coord:[D,o?0:-100],symbol:"pin",symbolSize:35,itemStyle:{color:"#ff5252"},label:{show:!0,formatter:"💥爆仓",fontSize:14,color:"#ff5252",fontWeight:"bold",offset:[0,-15]}}],animation:!1}}:{}})});const g=o?"总价值（元）":"收益率（%）";let I=1/0,S=-1/0;a.forEach(h=>{h.data&&h.data.forEach(w=>{w!==null&&!isNaN(w)&&(I=Math.min(I,w),S=Math.max(S,w))})});const B=S-I;I=I-B*.1,S=S+B*.1;const k={backgroundColor:"transparent",tooltip:{trigger:"axis",backgroundColor:"rgba(17,24,39,0.95)",borderColor:"rgba(79,195,247,0.3)",textStyle:{color:"#e5e7eb",fontSize:12},formatter:function(h){const w=h.filter(y=>y.value!==null&&y.value!==void 0&&!y.seriesName.includes("预测"));if(w.length===0)return"";let $='<div style="font-weight:bold;margin-bottom:4px;">'+h[0].axisValue.replace("🔮","预测 ")+"</div>";const v=[...w].sort((y,b)=>(b.value||0)-(y.value||0));for(const y of v){const b=t.find(E=>E.label===y.seriesName&&E.isUser),D=b?"⭐ ":"",T=o?"¥"+Number(y.value).toLocaleString():(y.value>=0?"+":"")+y.value.toFixed(1)+"%";$+='<div style="display:flex;align-items:center;gap:6px;'+(b?"font-weight:bold;":"")+'"><span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:'+y.color+';"></span>'+D+y.seriesName+": "+T+"</div>"}return $}},legend:{bottom:0,textStyle:{color:"#9ca3af",fontSize:10},icon:"roundRect",itemWidth:12,itemHeight:8,data:t.map(h=>h.label)},grid:{left:"12%",right:"8%",top:"10%",bottom:"15%"},xAxis:{type:"category",data:m,axisLine:{lineStyle:{color:"#2d3d54"}},axisTick:{show:!1},axisLabel:{color:"#6b7280",fontSize:9,rotate:30},splitLine:{show:!1}},yAxis:{type:"value",name:g,nameLocation:"middle",nameGap:50,nameTextStyle:{color:"#9ca3af",fontSize:12},axisLine:{show:!0,lineStyle:{color:"#2d3d54"}},axisTick:{show:!0,lineStyle:{color:"#2d3d54"}},axisLabel:{color:"#6b7280",fontSize:10,formatter:o?h=>h>=1e4?(h/1e4).toFixed(1)+"万":h.toLocaleString():h=>h.toFixed(0)+"%"},splitLine:{lineStyle:{color:"rgba(45,61,84,0.3)"}},min:Math.floor(I),max:Math.ceil(S)},series:a};A.setOption(k,!0);const x=document.getElementById("forecast-section");x&&x.classList.toggle("hidden",d.length===0),window.addEventListener("resize",()=>A==null?void 0:A.resize())}function wt(t){var e,n,o,r,s,a,i,l,c,d,u,m,p,g;Ct=t,A&&(A.dispose(),A=echarts.init(document.getElementById("chart-returns")),jt()),(e=document.getElementById("chart-mode-pct"))==null||e.classList.toggle("active",t==="pct"),(n=document.getElementById("chart-mode-pct"))==null||n.classList.toggle("bg-neon-blue/20",t==="pct"),(o=document.getElementById("chart-mode-pct"))==null||o.classList.toggle("text-neon-blue",t==="pct"),(r=document.getElementById("chart-mode-pct"))==null||r.classList.toggle("border-neon-blue/30",t==="pct"),(s=document.getElementById("chart-mode-pct"))==null||s.classList.toggle("bg-dark-500/30",t!=="pct"),(a=document.getElementById("chart-mode-pct"))==null||a.classList.toggle("text-gray-400",t!=="pct"),(i=document.getElementById("chart-mode-pct"))==null||i.classList.toggle("border-dark-500",t!=="pct"),(l=document.getElementById("chart-mode-value"))==null||l.classList.toggle("active",t==="value"),(c=document.getElementById("chart-mode-value"))==null||c.classList.toggle("bg-neon-blue/20",t==="value"),(d=document.getElementById("chart-mode-value"))==null||d.classList.toggle("text-neon-blue",t==="value"),(u=document.getElementById("chart-mode-value"))==null||u.classList.toggle("border-neon-blue/30",t==="value"),(m=document.getElementById("chart-mode-value"))==null||m.classList.toggle("bg-dark-500/30",t!=="value"),(p=document.getElementById("chart-mode-value"))==null||p.classList.toggle("text-gray-400",t!=="value"),(g=document.getElementById("chart-mode-value"))==null||g.classList.toggle("border-dark-500",t!=="value")}function ee(t,e,n){const o=document.getElementById(t);if(!o)return;H&&H.dispose(),H=echarts.init(o);const r={backgroundColor:"transparent",tooltip:{backgroundColor:"rgba(17,24,39,0.95)",borderColor:"rgba(124,58,237,0.3)",textStyle:{color:"#e5e7eb"}},radar:{center:["50%","50%"],radius:"65%",indicator:e.dimensions.map(s=>({name:s,max:100})),axisName:{color:"#9ca3af",fontSize:11},splitArea:{areaStyle:{color:["rgba(79,195,247,0.02)","rgba(79,195,247,0.02)"]}},splitLine:{lineStyle:{color:"rgba(45,61,84,0.3)"}},axisLine:{lineStyle:{color:"rgba(45,61,84,0.5)"}}},series:[{type:"radar",data:[{value:e.values,name:n,areaStyle:{color:"rgba(124,58,237,0.15)"},lineStyle:{color:"#b388ff",width:2},itemStyle:{color:"#b388ff"},symbol:"circle",symbolSize:5}]}]};H.setOption(r,!0),window.addEventListener("resize",()=>H==null?void 0:H.resize())}function zt(){const t=document.getElementById("sector-pie");t&&(N&&N.dispose(),N=echarts.init(t))}function ne(t){if(!N)return;const e=["#4fc3f7","#69f0ae","#f0c060","#ff5252","#b388ff","#ff80ab","#18ffff"],n={backgroundColor:"transparent",tooltip:{trigger:"item",backgroundColor:"rgba(17,24,39,0.95)",borderColor:"rgba(79,195,247,0.3)",textStyle:{color:"#e5e7eb"},formatter:"{b}: {c}% ({d}%)"},series:[{type:"pie",radius:["50%","75%"],center:["50%","50%"],emphasis:{label:{fontSize:14,fontWeight:"bold"},scaleSize:8},label:{color:"#9ca3af",fontSize:11,formatter:`{b}
{c}%`},labelLine:{lineStyle:{color:"#4b5563"}},data:t.length>0?t:[{name:"未选择",value:100,itemStyle:{color:"#1f2937"}}],itemStyle:{borderColor:"#0a0e17",borderWidth:2,color:o=>e[o.dataIndex%e.length]}}]};N.setOption(n,!0)}function oe(){A==null||A.dispose(),A=null,H==null||H.dispose(),H=null,N==null||N.dispose(),N=null}function kt(t,e){if(t.length<e)return[];const n=new Array(t.length).fill(null);let o=0;for(let r=0;r<t.length;r++)o+=t[r],r>=e&&(o-=t[r-e]),r>=e-1&&(n[r]=parseFloat((o/e).toFixed(2)));return n}function se(t,e=14){if(t.length<e+1)return[];const n=new Array(t.length).fill(null),o=[],r=[];for(let i=1;i<t.length;i++){const l=t[i]-t[i-1];o.push(l>0?l:0),r.push(l<0?-l:0)}let s=o.slice(0,e).reduce((i,l)=>i+l,0)/e,a=r.slice(0,e).reduce((i,l)=>i+l,0)/e;for(let i=e;i<o.length;i++){if(a===0)n[i+1]=100;else{const l=s/a;n[i+1]=parseFloat((100-100/(1+l)).toFixed(1))}s=(s*(e-1)+o[i])/e,a=(a*(e-1)+r[i])/e}return n}let tt=[],R=[],Z="a-share",nt="all",$t=!1,Lt=!1,Mt=!1,Et=!1,Rt=!1;function K(){L.holdings=R.map(t=>({code:t.code,name:t.name,sector:t.sector,market:t.market,weight:t.weight}))}function Ht(){var n;R=[],K(),Z="a-share",nt="all",L.stocksData&&ae(L.stocksData),$t||(document.querySelectorAll(".market-tab").forEach(o=>{o.addEventListener("click",()=>{document.querySelectorAll(".market-tab").forEach(r=>r.classList.remove("active")),o.classList.add("active"),Z=o.dataset.market,document.getElementById("stock-search").value="",G()})}),$t=!0),Rt||((n=document.getElementById("btn-random"))==null||n.addEventListener("click",le),Rt=!0);const t=document.getElementById("stock-search");if(t&&!Mt){let o=null;t.addEventListener("input",()=>{clearTimeout(o),o=setTimeout(()=>{const r=t.value.trim().toLowerCase();r&&(Z="all",document.querySelectorAll(".market-tab").forEach(s=>s.classList.remove("active"))),G(r)},250)}),Mt=!0}Lt||(document.querySelectorAll(".period-btn").forEach(o=>{o.addEventListener("click",()=>{document.querySelectorAll(".period-btn").forEach(s=>s.classList.remove("active")),o.classList.add("active");const r=o.dataset.period;r==="custom"?(document.getElementById("custom-period-wrap").classList.remove("hidden"),L.period="custom"):(document.getElementById("custom-period-wrap").classList.add("hidden"),L.period=r)})}),Lt=!0);const e=document.getElementById("custom-months");e&&!Et&&(e.addEventListener("input",()=>{L.customMonths=parseInt(e.value)||18}),Et=!0),zt(),W()}function ae(t){tt=t.stocks,re(t.sectors),G(),zt()}function re(t){const e=document.getElementById("sector-filters");if(!e)return;e.innerHTML="";const n=document.createElement("button");n.className="sector-btn active",n.textContent="全部",n.addEventListener("click",()=>{nt="all",document.querySelectorAll(".sector-btn").forEach(o=>o.classList.remove("active")),n.classList.add("active"),G()}),e.appendChild(n),t.forEach(o=>{const r=document.createElement("button");r.className="sector-btn",r.textContent=o,r.addEventListener("click",()=>{nt=o,document.querySelectorAll(".sector-btn").forEach(s=>s.classList.remove("active")),r.classList.add("active"),G()}),e.appendChild(r)})}function G(t){const e=document.getElementById("stock-grid");if(!e)return;let n=tt;if(t){const o=t.toLowerCase();n=tt.filter(r=>r.name.toLowerCase().includes(o)||r.code.toLowerCase().includes(o)).slice(0,50)}else Z==="all"&&(Z="a-share"),n=tt.filter(o=>{const r=o.market===Z,s=nt==="all"||o.sector===nt;return r&&s});n.sort((o,r)=>r.marketCap-o.marketCap),e.innerHTML=n.map(o=>{var i;const r=R.find(l=>l.code===o.code),s=o.latestPrice;return`
    ...
    `}).join(""),e.querySelectorAll(".stock-card").forEach(o=>{o.addEventListener("click",r=>{r.target.closest(".stock-detail-btn")||Ot(o.dataset)})}),e.querySelectorAll(".stock-detail-btn").forEach(o=>{o.addEventListener("click",r=>{r.stopPropagation();const s=o.dataset.code;ie(s)})})}function ie(t){var p,g,I,S,B,k;const e=tt.find(x=>x.code===t);if(!e)return;const n=e.prices.slice(-60),o=Math.min(...n),r=Math.max(...n),s={"a-share":"A股",hk:"港股",us:"美股",index:"指数"},a=n[0],l=((n[n.length-1]-a)/a*100).toFixed(2),c=l>=0?"text-neon-red":"text-neon-green",d=l>=0?"+":"",u=l>=0?"#ff5252":"#69f0ae";for(let x=0;x<n.length;x+=10);const m=document.createElement("div");m.className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm",m.innerHTML=`
    <div class="bg-dark-800 rounded-2xl p-6 max-w-lg w-full mx-4 border border-dark-500 shadow-2xl animate-fade-in max-h-[90vh] overflow-y-auto">
      ...
      <div class="grid grid-cols-2 gap-3 mb-4">
        ...
        <div class="bg-dark-700/30 rounded-lg p-3">
          ...
          <div class="text-lg font-mono text-white">${((S=e.roe)==null?void 0:S.toFixed(1))||"--"}%</div>
  ...
  `,document.body.appendChild(m),(B=m.querySelector(".stock-modal-close"))==null||B.addEventListener("click",()=>m.remove()),(k=m.querySelector(".stock-modal-add"))==null||k.addEventListener("click",function(){const{code:x,name:h,sector:w,market:M}=this.dataset;Ot({code:x,name:h,sector:w,market:M}),m.remove()}),m.addEventListener("click",x=>{x.target===m&&m.remove()}),setTimeout(()=>{const x=document.getElementById(`tech-indicators-${e.code}`);if(!x)return;const h=e.prices.slice(-120),w=kt(h,20),M=kt(h,60),$=se(h,14);h[h.length-1];const v=w[w.length-1],y=M[M.length-1],b=$[$.length-1],D=v>y?"📈 多头排列":"📉 空头排列",T=b>70?"⚠️ 超买":b<30?"💡 超卖":"➖ 中性";x.innerHTML=`
      ...
      <div><div class="text-xs text-gray-500">趋势</div><div class="text-xs">${D}</div><div class="text-xs text-gray-500">${T}</div></div>
    `},100),setTimeout(()=>{const x=document.getElementById("stock-price-chart");if(x&&typeof echarts<"u"){const h=echarts.init(x),w={backgroundColor:"transparent",grid:{left:"3%",right:"3%",top:"5%",bottom:"3%",containLabel:!0},xAxis:{type:"category",data:n.map((M,$)=>$+1),axisLine:{lineStyle:{color:"#2d3d54"}},axisLabel:{color:"#6b7280",fontSize:10,interval:9,formatter:M=>`${M}日`},axisTick:{show:!1}},yAxis:{type:"value",scale:!0,axisLine:{show:!1},axisTick:{show:!1},axisLabel:{color:"#6b7280",fontSize:10,formatter:M=>"¥"+M.toFixed(0)},splitLine:{lineStyle:{color:"rgba(45,61,84,0.3)"}}},series:[{data:n,type:"line",smooth:!0,symbol:"none",lineStyle:{width:3,color:u},areaStyle:{color:new echarts.graphic.LinearGradient(0,0,0,1,[{offset:0,color:u+"40"},{offset:1,color:u+"00"}])}}],tooltip:{trigger:"axis",backgroundColor:"rgba(17,24,39,0.95)",borderColor:u,textStyle:{color:"#e5e7eb",fontSize:12},formatter:M=>{const $=M[0].value;return`<div style="font-weight:bold">第${M[0].axisValue}天</div><div>价格: ¥${$.toFixed(2)}</div>`}}};h.setOption(w),window.addEventListener("resize",()=>h.resize())}},100),m.addEventListener("click",x=>{x.target===m&&m.remove()})}function Ot({code:t,name:e,sector:n,market:o}){var a,i;const r=R.findIndex(l=>l.code===t);if(r>=0)R.splice(r,1);else if(R.length<10)R.push({code:t,name:e,sector:n,market:o,weight:0});else{showToast("最多选择10只成分股","error");return}xt(),K();const s=(i=(a=document.getElementById("stock-search"))==null?void 0:a.value)==null?void 0:i.trim();G(s||void 0),W(),Y(),V()}function xt(){if(R.length===0)return;const t=Math.floor(100/R.length),e=100-t*R.length;R.forEach((o,r)=>{o.weight=t+(r<e?1:0)});const n=R.reduce((o,r)=>o+r.weight,0);n!==100&&R.length>0&&(R[0].weight+=100-n)}function W(){var r;const t=document.getElementById("selected-list"),e=document.getElementById("weight-sum");if(R.length===0){t.innerHTML='<span class="text-gray-500">请从上方选择股票</span>',e.textContent="合计: 0%";return}t.innerHTML=R.map((s,a)=>`
  ...
  `).join("");const n=R.reduce((s,a)=>s+a.weight,0);e.textContent=`合计: ${n}%`,e.className=n===100?"text-sm font-mono text-neon-green":"text-sm font-mono text-neon-red";const o=((r=document.getElementById("lock-weights"))==null?void 0:r.checked)||!1;t.querySelectorAll('[data-action="weight"]').forEach(s=>{s.addEventListener("input",a=>{const i=parseInt(s.dataset.index);R[i].weight=parseInt(a.target.value);const l=t.querySelector(`[data-action="weight-input"][data-index="${i}"]`);l&&(l.value=a.target.value),o?(K(),W(),Y(),V()):It(i,parseInt(a.target.value))}),s.addEventListener("change",a=>{if(!o)return;const i=parseInt(s.dataset.index);R[i].weight=parseInt(a.target.value),K(),W(),Y(),V()})}),t.querySelectorAll('[data-action="weight-input"]').forEach(s=>{s.addEventListener("change",a=>{const i=parseInt(s.dataset.index);let l=parseInt(a.target.value)||1;l=Math.max(1,Math.min(95,l)),R[i].weight=l;const c=t.querySelector(`[data-action="weight"][data-index="${i}"]`);c&&(c.value=l),o?(K(),W(),Y(),V()):It(i,l)})}),t.querySelectorAll('[data-action="remove"]').forEach(s=>{s.addEventListener("click",()=>{const a=parseInt(s.dataset.index);R.splice(a,1),xt(),K(),G(),W(),Y(),V()})})}function It(t,e){const n=R.filter((a,i)=>i!==t);if(n.length===0)return;R[t].weight=e;const o=100-e,r=n.reduce((a,i)=>a+i.weight,0);if(r===0){const a=Math.floor(o/n.length);n.forEach(l=>l.weight=a);const i=n.reduce((l,c)=>l+c.weight,0);n[0].weight+=o-i}else{const a=o/r;let i=0;n.forEach((d,u)=>{d.weight=Math.max(1,Math.round(d.weight*a)),i+=d.weight});let l=o-i,c=0;for(;l!==0&&c<20;){c++;for(const d of n)if(l>0?(d.weight++,l--):l<0&&d.weight>1&&(d.weight--,l++),l===0)break}l!==0&&n.length>0&&(n[0].weight=Math.max(1,n[0].weight+l))}const s=R.reduce((a,i)=>a+i.weight,0);s!==100&&R.length>0&&(R[0].weight+=100-s),K(),W(),Y(),V()}function Y(){const t={};R.forEach(n=>{t[n.sector]=(t[n.sector]||0)+n.weight});const e=Object.entries(t).map(([n,o])=>({name:n,value:o}));ne(e)}function le(){var n;R=[];const t=4+Math.floor(Math.random()*4),e=[...tt].sort(()=>Math.random()-.5);for(let o=0;o<Math.min(t,e.length);o++){const r=e[o];R.push({code:r.code,name:r.name,sector:r.sector,market:r.market,weight:0})}xt(),document.getElementById("stock-search").value="",Z="a-share",document.querySelectorAll(".market-tab").forEach(o=>o.classList.remove("active")),(n=document.querySelector('[data-market="a-share"]'))==null||n.classList.add("active"),G(),W(),Y(),V(),showToast(`🎲 随机选中 ${R.length} 只股票，看看运气如何？`)}function qt(){return R.map(t=>({code:t.code,weight:t.weight}))}function ce(){return L.period==="custom"?"custom"+(L.customMonths||18):L.period}let St=!1;function de(t){var i,l;const{results:e,amount:n,leverage:o}=t,r=n||1e5,s=o||1,a=[...e].sort((c,d)=>c.rank-d.rank);ue(a,r,s),te("chart-returns",a,r,s),me(a,r,s),St||((i=document.getElementById("chart-mode-pct"))==null||i.addEventListener("click",()=>wt("pct")),(l=document.getElementById("chart-mode-value"))==null||l.addEventListener("click",()=>wt("value")),St=!0)}function ue(t,e,n){const o=document.getElementById("ranking-table");if(!o)return;const r=["🥇","🥈","🥉"];o.innerHTML=t.map((s,a)=>{const i=s.isUser,l=a<3?r[a]:s.rank,c=s.totalReturn>=0?"text-neon-red":"text-neon-green",d=i?"user-highlight":"",u=s.totalReturn*n,m=parseFloat(Math.max(-100,u).toFixed(1)),p=Math.round(e*m/100),g=(m>=0?"+":"")+Number(p).toLocaleString(),I=parseFloat((s.maxDrawdown*n).toFixed(1));let S="";if(!i&&s.holdingsDetail&&s.holdingsDetail.length>0){const B=s.holdingsDetail.map(k=>`<div class="flex justify-between text-xs py-1">
      ...
      `}return`
      <div class="rank-row ${d} animate-slide-up" style="animation-delay: ${a*.08}s">
    ...
    `}).join(""),window.toggleHoldings||(window.toggleHoldings=function(s){const a=document.getElementById(`holdings-${s}`);a&&a.classList.toggle("hidden")})}function me(t,e,n){const o=document.getElementById("metrics-table");if(!o)return;const r=["基金","累计收益","年化收益","最大回撤","夏普比率","胜率"],s=t.map(a=>{const i=a.totalReturn>=0?"metric-up":"metric-down",l=a.totalReturn*n,c=parseFloat(Math.max(-100,l).toFixed(1)),d="★".repeat(a.fundRating||0)+"☆".repeat(5-(a.fundRating||0)),u=a.isUser?`
      ...
      <table class="metrics-table w-full text-xs">
        <thead>
          <tr class="border-b border-dark-500/30">
            ${r.map(a=>`<th class="px-3 py-2 text-left font-medium whitespace-nowrap cursor-help" title="${ge(a)}">${a}</th>`).join("")}
          </tr>
        </thead>
        <tbody>
          ${s}
  ...
  `}function ge(t){return{基金:"基金名称",累计收益:"回测期内的总收益率",年化收益:"按年计算的收益率",最大回撤:"从高点到低点的最大亏损幅度",夏普比率:"风险调整后收益，>1优秀",胜率:"盈利交易日占比"}[t]||t}window.showMetricDetail||(window.showMetricDetail=function(t){var l;const n={sharpe:{title:"夏普比率 (Sharpe Ratio)",content:`夏普比率 = (年化收益率 - 无风险利率) / 年化波动率
...
• ★☆☆☆☆：<1分，需谨慎`}}[t];if(!n)return;(l=document.querySelector(".metric-modal-overlay"))==null||l.remove();const o=document.createElement("div");o.className="metric-modal-overlay fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm",o.innerHTML=`
    ...
    `,document.body.appendChild(o);const r=o.querySelector(".metric-modal-close"),s=o.querySelector("button"),a=()=>o.remove();r==null||r.addEventListener("click",a),s==null||s.addEventListener("click",a),o.addEventListener("click",c=>{c.target===o&&a()});const i=c=>{c.key==="Escape"&&(a(),document.removeEventListener("keydown",i))};document.addEventListener("keydown",i)});function he(t,e=null){const{styleTag:n,matchPerson:o,matchPersonDesc:r,matchPersonOrg:s,metrics:a,radarData:i,commentary:l}=t,c=document.getElementById("diagnosis-tag");c&&(c.innerHTML=`
    ...
    `),ee("chart-radar",i,"你的基金");const u=document.getElementById("commentary-text");u&&(e!=null&&e.results&&e.results.length>0?Nt(u,e.results,e.errors):e!=null&&e.loading?pe(u):e!=null&&e.errors&&e.errors.length>0?xe(u,l,e.errors):ve(u,l)),be(a)}function fe(t){const e=document.getElementById("commentary-text");e&&(t.results&&t.results.length>0?Nt(e,t.results,t.errors):t.errors&&t.errors.length>0&&e.innerHTML.includes("loading-dots")&&ye(e,t.errors))}function Nt(t,e,n){let o=e.map((r,s)=>`<div class="llm-result mb-3">
...
`).map(i=>i.trim()).filter(Boolean).map(i=>`<p style="margin-bottom:10px;line-height:1.8;">${i.replace(/\n/g,"<br>")}</p>`).join("")}</div>
    ...
    </div>`),t.innerHTML=o}function pe(t){t.innerHTML=`
  ...
  `}function xe(t,e,n){let o="";if(e){const r=e.split(`
...
`).map(s=>s.trim()).filter(Boolean);o+=r.map((s,a)=>'<p style="margin-bottom:'+(a<r.length-1?"12px":"0")+';line-height:1.8;">'+s+"</p>").join("")}else o+='<p class="text-gray-400 text-sm">暂无可用的点评内容。</p>';o+='<div class="mt-3 text-right text-xs text-gray-500">📋 离线模板点评 · 仅供参考</div>',o+=`<div class="mt-4 p-3 bg-dark-600/30 rounded-lg border border-dark-500/30">
  ...
  </div>`,t.innerHTML=o}function ve(t,e){if(!e){t.innerHTML='<p class="text-gray-400 text-sm">暂无可用的点评内容。</p>';return}const n=e.split(`

`).map(o=>o.trim()).filter(Boolean);t.innerHTML=n.map((o,r)=>'<p style="margin-bottom:'+(r<n.length-1?"12px":"0")+';line-height:1.8;">'+o+"</p>").join(""),t.innerHTML+='<div class="mt-3 text-right text-xs text-gray-500">📋 离线模板点评 · 仅供参考</div>'}function ye(t,e){t.innerHTML=`
    <div class="py-2">
  ...
  `}function be(t){const e=document.getElementById("ai-commentary");if(!e)return;let n=document.getElementById("metrics-summary");n&&n.remove(),n=document.createElement("div"),n.id="metrics-summary";const o=(a,i)=>{if(a==null||isNaN(a))return"-";const l=Math.pow(10,i);return Math.round(a*l)/l},r="★".repeat(t.fundRating||0)+"☆".repeat(5-(t.fundRating||0)),s=t.fundRating>=4?"text-gold-400":t.fundRating>=3?"text-neon-blue":"text-gray-400";n.innerHTML=`
  ...
  `,e.appendChild(n)}const F=252;function z(t,e){const n=Math.pow(10,e);return Math.round(t*n)/n}const we={"3m":Math.floor(F/4),"6m":Math.floor(F/2),"1y":F,"3y":F*3,"5y":F*5,"10y":F*10};function ke(t){if(t.startsWith("custom")){const e=parseInt(t.replace("custom",""))||18;return Math.floor(F*e/12)}return we[t]||F}function Wt(t,e,n){var bt;const o=Math.min(ke(n),F*10),r={};t.stocks.forEach(f=>{r[f.code]=f});const s=[],a=100,i=((bt=r[e[0].code])==null?void 0:bt.prices.length)||F*5;for(let f=o;f>0;f--){const P=i-f;let ot=0;for(const st of e){const at=r[st.code];if(!at||P>=at.prices.length)continue;const Yt=at.prices[P],Xt=at.prices[i-o],Jt=st.weight/100;ot+=Jt*(Yt/Xt)}s.push(parseFloat((a*ot).toFixed(4)))}const l=s[s.length-1],c=z((l-a)/a*100,2);let d=0,u=s[0];for(const f of s){f>u&&(u=f);const P=(u-f)/u*100;P>d&&(d=P)}d=z(d,2);const m=o/F,p=z((Math.pow(l/a,1/m)-1)*100,2),g=[];for(let f=1;f<s.length;f++)g.push((s[f]-s[f-1])/s[f-1]);const I=g.reduce((f,P)=>f+P,0)/g.length,S=g.reduce((f,P)=>f+Math.pow(P-I,2),0)/g.length,B=Math.sqrt(S),k=z(B*Math.sqrt(F)*100,2),x=.02,h=k>0?z((p/100-x)/(k/100),2):0,w=g.filter(f=>f<0),M=w.length>0?Math.sqrt(w.reduce((f,P)=>f+Math.pow(P-w.reduce((ot,st)=>ot+st,0)/w.length,2),0)/w.length):0,$=M>0?z((p/100-x)/(M*Math.sqrt(F)),2):0,v=z((p/100-x)/1,2),y=g.map(f=>f-x/F),b=Math.sqrt(y.reduce((f,P)=>f+P*P,0)/y.length)*Math.sqrt(F),D=b>0?z((p/100-x)/b,2):0,T=d>0?z(p/d,2):0,E=g.filter(f=>f>0).length,C=z(E/g.length*100,1),ct=g.filter(f=>f>0).reduce((f,P)=>f+P,0)/g.filter(f=>f>0).length||0,U=Math.abs(g.filter(f=>f<0).reduce((f,P)=>f+P,0)/g.filter(f=>f<0).length)||0,dt=U>0?z(ct/U,2):0;let j=0,O=[];h>=1.5?(j+=2,O.push("夏普比率优秀")):h>=1?(j+=1.5,O.push("夏普比率良好")):h>=.5&&(j+=1,O.push("夏普比率一般")),d<=10?(j+=1.5,O.push("回撤控制优秀")):d<=20?(j+=1,O.push("回撤控制良好")):d<=30&&(j+=.5),p>=20?(j+=1.5,O.push("收益表现优秀")):p>=10?(j+=1,O.push("收益表现良好")):p>=5&&(j+=.5),C>=60&&(j+=.5,O.push("胜率较高")),j=Math.min(5,Math.max(1,Math.round(j)));let ut="中";d<=15&&k<=20?ut="低":(d>=30||k>=40)&&(ut="高");const yt=Math.max(1,Math.floor(s.length/50)),mt=[],gt=[];for(let f=0;f<s.length;f+=yt)mt.push(s[f]),gt.push(f);(s.length-1)%yt!==0&&(mt.push(s[s.length-1]),gt.push(s.length-1));const _t=new Date,ht=new Date(_t);ht.setDate(ht.getDate()-o);const Kt=gt.map(f=>{const P=new Date(ht);return P.setDate(P.getDate()+f),P.getMonth()+1+"/"+P.getDate()});return{name:"user",label:"你的基金",isUser:!0,totalReturn:c,annualizedReturn:p,annualizedVol:k,maxDrawdown:d,sharpeRatio:h,sortinoRatio:$,treynorRatio:v,informationRatio:D,calmarRatio:T,profitLossRatio:dt,winRate:C,fundRating:j,ratingReasons:O,riskLevel:ut,initialValue:a,finalValue:l,chartData:mt,dateLabels:Kt,days:o,holdings:e.map(f=>{const P=r[f.code];return{code:f.code,name:(P==null?void 0:P.name)||f.code,weight:f.weight}})}}function $e(t,e){const n=[];return n.push(...Le(t,e)),n.push(...Me(t,e)),n}function q(t,e){const n=Math.pow(10,e);return Math.round(t*n)/n}function Le(t,e){const n=t.stocks,o=n.filter(c=>c.market==="a-share").sort((c,d)=>d.marketCap-c.marketCap).slice(0,20),r=o.map(c=>({code:c.code,weight:q(100/o.length,1)})),s=n.filter(c=>c.market==="a-share"&&(c.sector==="科技"||c.sector==="医药"||c.sector==="新能源")).filter(c=>c.marketCap<5e3).slice(0,15),a=s.map(c=>({code:c.code,weight:q(100/s.length,1)})),i=n.filter(c=>c.market==="us"&&c.sector==="科技").sort((c,d)=>d.marketCap-c.marketCap).slice(0,10),l=i.map(c=>({code:c.code,weight:q(100/i.length,1)}));return[X("benchmark-csi300","沪深300","A股大盘蓝筹基准","🇨🇳",r,t,e),X("benchmark-gem","创业板指","A股成长创新基准","🇨🇳",a,t,e),X("benchmark-nasdaq","纳斯达克100","美股科技龙头基准","🇺🇸",l,t,e)]}function Me(t,e){const n=t.stocks,o=n.filter(u=>u.pe>0&&u.pe<25&&u.dividendYield>2).filter(u=>u.sector==="消费"||u.sector==="金融").sort((u,m)=>m.dividendYield-u.dividendYield).slice(0,8),r=o.map(u=>({code:u.code,weight:q(100/o.length,1)})),s=n.filter(u=>u.revenueGrowth>10).filter(u=>u.sector==="科技"||u.sector==="医药"||u.sector==="新能源").sort((u,m)=>m.revenueGrowth-u.revenueGrowth).slice(0,8),a=s.map(u=>({code:u.code,weight:q(100/s.length,1)})),i=n.map(u=>{const m=u.prices,p=m[m.length-1],g=m[Math.max(0,m.length-63)];return{...u,momentum:q((p-g)/g*100,2)}}).sort((u,m)=>m.momentum-u.momentum).slice(0,8),l=i.map(u=>({code:u.code,weight:q(100/i.length,1)})),c=n.filter(u=>u.roe>5).map(u=>{const m=u.prices,p=m[m.length-1],g=m[Math.max(0,m.length-63)];return{...u,change:q((p-g)/g*100,2)}}).sort((u,m)=>u.change-m.change).slice(0,8),d=c.map(u=>({code:u.code,weight:q(100/c.length,1)}));return[X("ai-value","🐻 价值大师","深度价值投资","🐻",r,t,e),X("ai-growth","🐂 成长猎手","激进成长投资","🐂",a,t,e),X("ai-momentum","🐎 趋势追踪","动量交易策略","🐎",l,t,e),X("ai-reverse","🦉 逆向投资","超跌反转策略","🦉",d,t,e)]}function X(t,e,n,o,r,s,a){const i=Wt(s,r,a);return i.name=t,i.label=e,i.description=n,i.icon=o,i.isUser=!1,i.isBenchmark=t.startsWith("benchmark-"),i.holdingsDetail=r.map(l=>{const c=s.stocks.find(d=>d.code===l.code);return{code:l.code,name:(c==null?void 0:c.name)||l.code,weight:l.weight,sector:(c==null?void 0:c.sector)||"未知",market:(c==null?void 0:c.market)||"未知"}}),i}const rt=[{id:"jiucai",emoji:"🥬",name:"韭菜本菜",matchPerson:"每一个在市场里交过学费的人",personDesc:"初代股民集体回忆",personOrg:"",condition:t=>t.totalReturn<0&&t.concentration>.5},{id:"foxi",emoji:"🧘",name:"佛系躺平派",matchPerson:"但斌",personDesc:"「时间的玫瑰」——买了就当忘了",personOrg:"东方港湾董事长",condition:t=>t.turnover<.3&&t.bluechipRatio>.6},{id:"jiuxiang",emoji:"🍶",name:"酱香科技研究员",matchPerson:"张坤",personDesc:"易方达蓝筹精选掌舵人",personOrg:"易方达基金",condition:t=>(t.sectorWeights.消费||0)>30},{id:"yaoyao",emoji:"💊",name:"医药葛兰分兰",matchPerson:"葛兰",personDesc:"中欧医疗健康，医药赛道信仰者",personOrg:"中欧基金",condition:t=>(t.sectorWeights.医药||0)>40},{id:"ark",emoji:"🚀",name:"ARK中国分K",matchPerson:"Cathie Wood",personDesc:"ARK Invest创始人",personOrg:"ARK Invest",condition:t=>(t.sectorWeights.科技||0)>50&&t.turnover>.5},{id:"buffett",emoji:"👴",name:"巴菲特传人",matchPerson:"Warren Buffett",personDesc:"价值投资灯塔",personOrg:"伯克希尔·哈撒韦",condition:t=>(t.sectorWeights.消费||0)+(t.sectorWeights.金融||0)>50&&t.turnover<.3&&t.roe>15},{id:"diamond",emoji:"🦍",name:"钻石手",matchPerson:"WSB散户大军",personDesc:"「Diamond Hands」——回撤50%也绝不割肉",personOrg:"Reddit r/wallstreetbets",condition:t=>t.maxDrawdown>25&&t.turnover<.3},{id:"wolf",emoji:"🐺",name:"华尔街之狼",matchPerson:"各路游资大佬",personDesc:"高频交易，主打一个刺激",personOrg:"龙虎榜常客",condition:t=>t.turnover>.8},{id:"national",emoji:"🏛️",name:"国家队在逃成员",matchPerson:"社保基金/汇金",personDesc:"银行+央企+蓝筹，稳如泰山",personOrg:"全国社保基金理事会",condition:t=>(t.sectorWeights.金融||0)>40&&t.annualizedVol<20&&t.roe>10},{id:"global",emoji:"🌍",name:"全球宏观玩家",matchPerson:"Ray Dalio",personDesc:"桥水基金创始人",personOrg:"桥水基金",condition:t=>t.crossMarket&&t.marketCount>=3},{id:"growth",emoji:"🌱",name:"成长股猎人",matchPerson:"朱少醒",personDesc:"富国天惠，15年20倍的公募传奇",personOrg:"富国基金",condition:t=>(t.sectorWeights.科技||0)>30&&t.revenueGrowth>20},{id:"balanced",emoji:"⚖️",name:"均衡配置达人",matchPerson:"谢治宇",personDesc:"兴全合润，不偏科的均衡派代表",personOrg:"兴证全球基金",condition:t=>t.maxSectorWeight<35&&t.stockCount>=6}];function et(t,e){if(t==null||isNaN(t))return t;const n=Math.pow(10,e);return Math.round(t*n)/n}function Ee(t,e,n,o){var y;const s=Object.entries(e.sectorWeights||{}).sort((b,D)=>D[1]-b[1])[0]||["未知",0];Object.entries(e.marketWeights||{}).sort((b,D)=>D[1]-b[1]);const a={"a-share":"A股",hk:"港股",us:"美股",index:"指数ETF"},i=[],l={},c={};n.forEach(b=>{const D=o[b.code];D&&(l[D.market]=(l[D.market]||0)+b.weight,c[D.sector]=(c[D.sector]||0)+b.weight)});const d=Object.entries(l).sort((b,D)=>D[1]-b[1]),u=Object.entries(c).sort((b,D)=>D[1]-b[1]),m=(y=d[0])==null?void 0:y[0],p=d.length,g=e.leverage||1,I=e.maxDrawdown>=100||e.totalReturn<=-100,S=g>3,B=e.totalReturn<-50,k=e.totalReturn<-20&&e.totalReturn>=-50,x=e.totalReturn<0&&e.totalReturn>=-20,h=e.totalReturn>=0&&e.totalReturn<10,w=e.totalReturn>=10&&e.totalReturn<50,M=e.totalReturn>=50;let $="";if(I?S?$=`💥 **爆仓警告！** 你使用了${g}x杠杆，最终回撤${e.maxDrawdown.toFixed(1)}%，本金几乎归零。这不是投资，这是赌博！高杠杆+重仓=自杀式操作。`:$=`💥 **巨额亏损！** 最大回撤${e.maxDrawdown.toFixed(1)}%，几乎亏光所有本金。你的选股或择时出现了严重问题。`:B?S?$=`📉 **高杠杆惨案！** ${g}x杠杆放大了亏损，最终收益${e.totalReturn.toFixed(1)}%。杠杆是双刃剑，这次你被割伤了。`:$=`📉 **深度套牢！** 亏损${Math.abs(e.totalReturn).toFixed(1)}%，持仓体验极差。建议重新审视每只股票的基本面。`:k?$=`😰 **投资失利！** 亏损${Math.abs(e.totalReturn).toFixed(1)}%，虽然没到爆仓程度，但也足够肉疼。复盘一下原因？`:x?$=`🤔 **白忙一场！** 亏了${Math.abs(e.totalReturn).toFixed(1)}%，承担了风险却没得到回报。`:h?$=`🙂 **小赚一笔！** 盈利${e.totalReturn.toFixed(1)}%，虽然不多但好歹是正收益。`:w?$=`😊 **稳健盈利！** 收益${e.totalReturn.toFixed(1)}%，回撤${e.maxDrawdown.toFixed(1)}%，这是真正的投资能力！`:M&&(S?$=`🚀 **杠杆暴利！** ${g}x杠杆+${e.totalReturn.toFixed(1)}%收益=暴富神话！但别飘，见好就收。`:$=`🌟 **投资大师！** 收益${e.totalReturn.toFixed(1)}%，这是巴菲特级别的表现！`),i.push($),i.push(`
📊 **持仓诊断**：`),u.length>0){const b=u[0],D=b[1]>60?`重仓${b[0]}(${b[1].toFixed(0)}%)，集中度极高，风险集中。`:b[1]>40?`${b[0]}(${b[1].toFixed(0)}%)占比偏高。`:"行业分布较均衡。";i.push(`• ${D}`)}p===1?i.push(`• 全仓${a[m]||m}，单一市场风险集中。`):i.push(`• 跨${p}个市场配置，分散了风险。`),e.stockCount<=2?i.push(`• 仅${e.stockCount}只标的，集中度极高，押注式投资风险极大。`):e.stockCount>=8?i.push(`• ${e.stockCount}只标的，可能过于分散。`):i.push(`• ${e.stockCount}只标的，集中度适中。`),g>1&&(i.push(`
...
💡 **专属建议**：`);const v=[];return I?(v.push("🚨 立即退出所有杠杆仓位，本金没了就什么都没了。"),v.push("📚 建议先学习《聪明的投资者》等经典书籍。"),v.push("🎮 先用模拟盘练习至少3个月。")):B||k?(v.push("🛑 暂停加仓，不要继续摊低成本。"),v.push("🔍 仔细分析每只股票的买入逻辑。"),S&&v.push("📉 降低杠杆至1x或2x。")):x?v.push("🤔 微调策略，优化选股标准。"):h?v.push("📊 加入债券ETF等低风险资产平滑曲线。"):(w||M)&&v.push("💰 适当减仓，锁定部分利润。"),s[1]>60&&v.push(`🔄 ${s[0]}占比过高，建议减仓分散。`),p===1&&!I&&v.push("🌍 建议配置其他市场分散风险。"),e.maxDrawdown>30&&!I&&v.push("🛡️ 设置止损线（如-15%）并严格执行。"),i.push(...v.map((b,D)=>`${D+1}. ${b}`)),i.push(`
🎯 **总结**：`),I?i.push("这次投资以爆仓告终。记住这次教训，重建本金，重新出发。💪"):B||k?i.push("这次投资虽然亏损，但经验比金钱更重要。🌱"):x?i.push("基本持平，小幅优化就能扭亏为盈。📚"):h?i.push("小赚是不错的开始，继续优化。🐢"):w?i.push("不错的收益！保持并持续优化。🏆"):M&&i.push("卓越的表现！保持学习、控制风险。🌟"),i.join(`
`)}function Vt(t,e,n){const o={};t.stocks.forEach(k=>{o[k.code]=k});const r={},s={};let a=0,i=0,l=0,c=0;e.forEach(k=>{const x=o[k.code];if(!x)return;const h=k.weight/100;r[x.sector]=(r[x.sector]||0)+k.weight,s[x.market]=(s[x.market]||0)+k.weight,a+=x.revenueGrowth*h,i+=x.roe*h,l+=x.pe*h,x.marketCap>3e3&&c++});const d=e.length<=5?.7:e.length<=7?.4:.25,m=(r.科技||0)>40?.6+Math.random()*.2:.2+Math.random()*.3,p={totalReturn:n.totalReturn,annualizedReturn:n.annualizedReturn,annualizedVol:n.annualizedVol,maxDrawdown:n.maxDrawdown,sharpeRatio:n.sharpeRatio,sortinoRatio:n.sortinoRatio,informationRatio:n.informationRatio,calmarRatio:n.calmarRatio,profitLossRatio:n.profitLossRatio,winRate:n.winRate,fundRating:n.fundRating,ratingReasons:n.ratingReasons,riskLevel:n.riskLevel,leverage:n.leverage,sectorWeights:r,marketWeights:s,concentration:d,turnover:m,revenueGrowth:parseFloat(a.toFixed(1)),roe:parseFloat(i.toFixed(2)),pe:parseFloat(l.toFixed(2)),bluechipRatio:parseFloat((c/e.length).toFixed(2)),maxSectorWeight:parseFloat(Math.max(...Object.values(r)).toFixed(1)),stockCount:e.length,crossMarket:Object.keys(s).length>=2,marketCount:Object.keys(s).length};let g=null,I=0;for(const k of rt)if(k.condition(p)){const x=k.id==="jiucai"?5:k.id==="global"?3:1;x>I&&(I=x,g=k)}g||(g=rt.find(k=>k.id==="balanced")||rt[rt.length-1]);const S=Ee(g,p,e,o),B={dimensions:["年化收益","风险控制","行业集中度","跨市场配置","选股ROE"],values:[et(Math.min(100,Math.max(0,p.annualizedReturn+50)),0),et(Math.min(100,Math.max(0,100-p.annualizedVol)),0),et(Math.min(100,Math.max(0,p.maxSectorWeight)),0),et(Math.min(100,Math.max(0,Object.keys(s).length*30)),0),et(Math.min(100,Math.max(0,p.roe*1.5)),0)]};return{styleTag:`${g.emoji} ${g.name}`,matchPerson:g.matchPerson,matchPersonDesc:g.personDesc,matchPersonOrg:g.personOrg||"",styleId:g.id,metrics:p,radarData:B,commentary:S}}const Dt=window.LLM_CONFIG||{},it=Dt.primary||Dt.eastmoney||{baseUrl:"",apiKey:"",model:""};let J=null,Q=[],Gt=!1;const Pt=3e4;function Re(t){return Q=[],J=Pe(Se(t)).then(e=>e?[{model:"AI",text:e}]:null),J}async function Bt(){if(!J)return{results:null,errors:[{api:"System",error:"未发起请求"}],loading:!1};let t=null;const e=new Promise(o=>{setTimeout(()=>{o({stillLoading:!0})},100)}),n=await Promise.race([J,e]);return n&&n.stillLoading?t=await J:t=n,J=null,!t||t.length===0?{results:null,errors:[...Q],loading:!1}:{results:t,errors:[...Q],loading:!1}}function Ie(){return J!==null}function Se(t){const{styleTag:e,matchPerson:n,matchPersonDesc:o,matchPersonOrg:r,metrics:s}=t,a=Object.entries(s.sectorWeights||{}).map(([u,m])=>`${u}${m.toFixed(0)}%`).join("、"),i=Object.entries(s.marketWeights||{}).map(([u,m])=>`${{"a-share":"A股",hk:"港股",us:"美股",index:"指数ETF"}[u]||u}${m.toFixed(0)}%`).join("、"),c=(s.annualizedReturn||0)>=0;Gt=c;const d=c?"赚了别藏着，去东方财富股吧晒晒操作，让韭菜们学学！":"亏了别慌，下载东方财富APP，权威·专业·及时，帮你找回场子！";return`你是东方财富的资深基金经理，为用户点评投资组合。
...
${d}`}function De(t){if(!t)return"";const e=[/最终回答[：:]\s*/,/最终点评[：:]\s*/,/以下是点评[：:]\s*/,/点评如下[：:]\s*/,/回复[：:]\s*\n/];for(const r of e){const s=t.match(r);if(s){const a=t.slice(s.index+s[0].length).trim();if(a.length>50)return a}}const n=t.split(/\n\n+/).filter(r=>r.trim());if(n.length>=3)for(let r=n.length-1;r>=0;r--){const s=n[r].trim();if(!/^(我们|首先|需要|用户|任务|好的|让我|我来|根据|以上|下面|现在)/.test(s)&&s.length>20)return n.slice(r).join(`

`)}const o=t.replace(/^.*?我们需要.*?\n/s,"").trim();return o.length>50?o:t.trim()}async function Pe(t){var r,s;if(!it.apiKey)return Q.push({api:"API",error:"未配置 apiKey（请创建 config.js 并填入 Key）"}),null;const e=it.baseUrl||"/api",n=new AbortController,o=setTimeout(()=>n.abort(),Pt);try{const a=await fetch(`${e}/chat/completions`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${it.apiKey}`},body:JSON.stringify({model:it.model,messages:[{role:"system",content:"你是东方财富的资深基金经理。直接输出点评正文，禁止输出思考过程。少用引号，适度emoji，结尾自然引导到东方财富产品。"},{role:"user",content:t}],max_tokens:2e3,temperature:.8}),signal:n.signal});if(!a.ok){const d=await a.text().catch(()=>"");return Q.push({api:"API",error:`HTTP ${a.status}${d?": "+d.slice(0,200):""}`}),null}const l=(s=(r=(await a.json()).choices)==null?void 0:r[0])==null?void 0:s.message;let c=((l==null?void 0:l.content)||"").trim();return!c&&(l!=null&&l.reasoning_content)&&(c=De(l.reasoning_content)),c&&!c.includes("东方财富")&&(c+=Gt?`

赚了别藏着，去东方财富股吧晒晒操作，让韭菜们学学！`:`

亏了别慌，下载东方财富APP，权威·专业·及时，帮你找回场子！`),c||Q.push({api:"API",error:"返回内容为空（模型未输出有效回复）"}),c||null}catch(a){const i=a.name==="AbortError"?`请求超时（${Pt/1e3}秒）。请检查：1) 是否在公司内网 2) Vite 代理是否正常 3) API 地址是否正确`:a.message||"网络错误";return Q.push({api:"API",error:i}),console.warn("[LLM]",a.message),null}finally{clearTimeout(o)}}const L={currentScreen:"builder",fundName:"",holdings:[],period:"1y",customMonths:18,backtestResults:null,stocksData:null,userResult:null,investAmount:1e5,leverage:1};let lt=null,ft=null;function Be(){const t=document.getElementById("particle-canvas");if(!t)return;const e=t.getContext("2d");let n=[];lt&&(cancelAnimationFrame(lt),lt=null),ft&&window.removeEventListener("resize",ft);function o(){t.width=window.innerWidth,t.height=window.innerHeight}o(),ft=o,window.addEventListener("resize",o);class r{constructor(){this.reset()}reset(){this.x=Math.random()*t.width,this.y=Math.random()*t.height,this.size=Math.random()*2+.5,this.speedX=(Math.random()-.5)*.3,this.speedY=(Math.random()-.5)*.3,this.opacity=Math.random()*.5+.1}update(){this.x+=this.speedX,this.y+=this.speedY,(this.x<0||this.x>t.width||this.y<0||this.y>t.height)&&this.reset()}draw(){e.beginPath(),e.arc(this.x,this.y,this.size,0,Math.PI*2),e.fillStyle=`rgba(79, 195, 247, ${this.opacity})`,e.fill()}}for(let a=0;a<80;a++)n.push(new r);function s(){e.clearRect(0,0,t.width,t.height),n.forEach(a=>{a.update(),a.draw()});for(let a=0;a<n.length;a++)for(let i=a+1;i<n.length;i++){const l=n[a].x-n[i].x,c=n[a].y-n[i].y,d=Math.sqrt(l*l+c*c);d<120&&(e.beginPath(),e.moveTo(n[a].x,n[a].y),e.lineTo(n[i].x,n[i].y),e.strokeStyle=`rgba(79, 195, 247, ${.08*(1-d/120)})`,e.lineWidth=.5,e.stroke())}lt=requestAnimationFrame(s)}s()}function vt(t){oe(),document.querySelectorAll(".screen").forEach(o=>o.classList.remove("active","hidden")),document.querySelectorAll(".screen").forEach(o=>o.classList.add("hidden"));const e=document.getElementById(`screen-${t}`);e&&(e.classList.remove("hidden"),e.classList.add("active"),e.scrollIntoView({behavior:"smooth",block:"start"})),L.currentScreen=t;const n=document.getElementById("header");n&&(n.style.display=t==="builder"?"":"none")}async function Ut(){try{const e=await fetch("./"+"stocks.json");if(!e.ok)throw new Error(`HTTP ${e.status}`);const n=await e.json();return n.stocks.forEach(o=>{o.latestPrice=o.prices[o.prices.length-1]}),L.stocksData=n,L.stocksData}catch(t){return console.error("Failed to load stocks:",t),null}}async function Ce(){var e,n,o,r;const t=document.getElementById("btn-start");t.disabled=!0,t.textContent="⏳ 回测计算中...";try{const s=qt(),a=ce();let i=parseFloat((e=document.getElementById("invest-amount"))==null?void 0:e.value)||1e5;i=Math.max(100,Math.min(1e8,i));const l=parseFloat((n=document.getElementById("leverage"))==null?void 0:n.value)||1;let c=L.fundName||((r=(o=document.getElementById("fund-name"))==null?void 0:o.value)==null?void 0:r.trim());c||(c=Ae(s,L.stocksData)),L.stocksData||await Ut();const d=Wt(L.stocksData,s,a);d.label=c,d.amount=i,d.leverage=l,L.userResult=d,L.holdings=s,L.investAmount=i,L.leverage=l;const u=$e(L.stocksData,a),m=[d,...u];m.sort((I,S)=>S.totalReturn-I.totalReturn),m.forEach((I,S)=>{I.rank=S+1}),L.backtestResults=m,vt("arena"),de({fundName:c,period:a,results:m,amount:i,leverage:l});const p={...d,totalReturn:d.totalReturn*l,maxDrawdown:d.maxDrawdown*l},g=Vt(L.stocksData,s,p);g.metrics.leverage=l,Re(g)}catch(s){ze("回测失败："+s.message,"error"),t.disabled=!1,t.textContent="⚡ 开始挑战"}}async function Fe(){vt("diagnosis");const t={...L.userResult,totalReturn:L.userResult.totalReturn*L.leverage,maxDrawdown:L.userResult.maxDrawdown*L.leverage},e=Vt(L.stocksData,L.holdings,t);e.metrics.leverage=L.leverage;const n=Ie();let o={results:null,errors:[],loading:n};n||(o=await Bt()),he(e,o),n&&(o=await Bt(),fe(o))}function Te(){L.fundName="",L.holdings=[],L.backtestResults=null,L.userResult=null,document.getElementById("fund-name").value="",document.getElementById("btn-start").disabled=!0,vt("builder"),Ht()}function Ae(t,e){var $;if(!t||t.length===0)return"我的基金";const n={};e&&e.stocks&&e.stocks.forEach(v=>{n[v.code]=v});const o={},r={};let s=!1,a=!1,i=!1,l=!1;t.forEach(v=>{const y=n[v.code];y&&(o[y.market]=(o[y.market]||0)+v.weight,r[y.sector]=(r[y.sector]||0)+v.weight,y.sector==="科技"&&(s=!0),y.sector==="金融"&&(a=!0),y.sector==="消费"&&(i=!0),y.sector==="医药"&&(l=!0))});const c=Object.entries(o).sort((v,y)=>y[1]-v[1]),d=(($=c[0])==null?void 0:$[0])||"a-share",u=c.length,m={"a-share":["华夏","国泰","南方","易方达","嘉实","博时","广发","富国"],hk:["港股","香港","恒生","中港","沪港深"],us:["纳斯达克","标普","美股","全球","海外"],index:["指数","ETF","被动"]};let p;u>=3?p=["全球","国际","环球","世界","跨市场"]:u===2?p=["沪港深","深港通","AH","中美","跨市场"]:p=m[d]||m["a-share"];let g=[];s&&t.length<=3?g=["创新","科技","成长","新兴","前沿","智能"]:a&&t.length<=3?g=["金融","价值","蓝筹","红利","稳健","精选"]:i&&t.length<=3?g=["消费","品质","生活","品牌","升级"]:l&&t.length<=3?g=["健康","医疗","生命","医药","生物"]:t.length>=8?g=["优选","精选","配置","均衡","多元","全能"]:t.length<=3?g=["聚焦","集中","核心","龙头","精选","优势"]:g=["成长","价值","均衡","轮动","趋势","精选","优选","灵活"];const I=["混合","股票","配置","优选","精选","成长","价值","稳健","进取","灵活"],S=p[Math.floor(Math.random()*p.length)],B=g[Math.floor(Math.random()*g.length)],k=I[Math.floor(Math.random()*I.length)],x=[S+B+k,S+k+B,B+k,S+B],h=x[Math.floor(Math.random()*x.length)],w=["超级","至尊","王者","巅峰","传奇","无敌","神级","霸道"];return w[Math.floor(Math.random()*w.length)]+h+"（您）"}async function je(){Be(),await Ut(),Ht();const t=document.getElementById("invest-amount");t&&(t.addEventListener("input",n=>{let o=parseInt(n.target.value)||1e5;o=Math.max(100,Math.min(1e8,o)),n.target.value&&parseInt(n.target.value)!==o&&(n.target.value=o)}),t.addEventListener("blur",n=>{let o=parseInt(n.target.value)||1e5;o=Math.max(100,Math.min(1e8,o)),n.target.value=o})),document.getElementById("btn-start").addEventListener("click",Ce),document.getElementById("btn-diagnosis").addEventListener("click",Fe),document.getElementById("btn-restart").addEventListener("click",Te),document.getElementById("fund-name").addEventListener("input",n=>{L.fundName=n.target.value.trim(),V()});const e=document.getElementById("leverage");e&&e.addEventListener("input",()=>{document.getElementById("leverage-display").textContent=e.value+"x"})}je();let pt=null;function ze(t,e="info"){const n=document.getElementById("toast-msg");n&&n.remove(),pt&&clearTimeout(pt);const o=e==="error"?"bg-red-500/90":"bg-green-500/90",r=document.createElement("div");r.id="toast-msg",r.className=`fixed top-4 left-1/2 -translate-x-1/2 ${o} text-white px-6 py-3 rounded-xl shadow-lg z-50 text-sm`,r.textContent=t,document.body.appendChild(r),pt=setTimeout(()=>{r.style.opacity="0",r.style.transition="opacity .3s",setTimeout(()=>r.remove(),300)},3e3)}function V(){const t=document.getElementById("btn-start");t&&(t.disabled=qt().length<1)}
